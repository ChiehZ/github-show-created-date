...
function insertCreatedTime(createdDate) {
  // 找到 Stars 和 Forks 的容器
  const statsContainer = document.querySelector("ul.pagehead-actions");
  if (!statsContainer) {
  ...
  // 創建新欄位
  const createdTimeLi = document.createElement("li");
  createdTimeLi.id = "repo-created-time";
  createdTimeLi.className = "d-inline-block mr-3";
  createdTimeLi.innerHTML = `
    <span class="d-inline-block">
      <svg class="octicon octicon-calendar" viewBox="0 0 16 16" width="16" height="16" aria-hidden="true">
  ...
  `;
  statsContainer.appendChild(createdTimeLi);
}